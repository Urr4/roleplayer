...
public class JpaRecordingAdapter implements RecordingRepository {
    ...
    @Override public Recording save(Recording recording) {
        RecordingEntity saved = repository.save(new RecordingEntity(recording.id(), recording.chronicleId(), recording.adventureId(), recording.source(), recording.status(), recording.startedAt(), recording.endedAt(), recording.audioObjectKey(), recording.transcriptObjectKey(), recording.errorMessage()));
